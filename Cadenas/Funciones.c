...
bool session(){
	...
	}while((strcmp(init1,user) != 0 || strcmp(init2,password) != 0) && cont < 3 );
...
//Los mismo que lo anterior (pero para nombres de personas como lo determina el programa)
...
//Los mismo que lo anterior (pero para apellidos de personas como lo determina el programa)
...
void MaxToMinV(char x[100]){
	...
	int vocal = 0;char a;
	char aux[100]; strcpy(aux,x);  strupr(aux);
...
//Mostrar el primer nombre en mayusculas de la persona con el segundo nombre mayor alfabeticamente
...
//Primera letra en mayúscula de los apellidos de la persona que tenga el primer apellido mayor alfabéticamente
void firstLetterUP(char x[100],char y[100],char z[100], char v[100]){
	...
	if (strcmp(x,y) > 0) {
		...
		letter[0] = x[0]; l[0] = x[0];
		letter[1] = z[0]; 
		strupr(letter);
		strcpy(helper1,strtok(x,l));
		l[0] = z[0];
		strcpy(helper2,strtok(z,l));
		printf("\n\t Apellidos: %c%s %c%s\n\n",letter[0],helper1,letter[1],helper2);
...
void sameSurName(char x[100],char y[100],char z[100], char v[100]){
	...
	if ((strcmp(x,y) == 0) && (strcmp(z,v) == 0)) {
		...
		printf("\n\t El primer apellido de la persona 1 es igual al segundo apellido de la persona 2");
		printf("\n\t Y el primer apellido de la persona 2 es igual al segundo apellido de la persona 1\n\n");
		printf("\n\t Apellidos: %s - %s - %s - %s",x,z,y,v);
	} else if(strcmp(x,y)==0){
		...
		printf("\n\t El primer apellido de la persona 1 es igual que el segundo apellido de la persona 2");
		printf("\n\t Apellidos: %s - %s",x,v);
	} else if(strcmp(y,z)==0){
		printf("\n\t El primer apellido de la persona 2 es igual que el segundo apellido de la persona 1");
		printf("\n\t Apellidos: %s - %s",y,z);
	} else{
		printf("\n\t Ninguno de los apellidos coincide con el de la otra persona\n\n");
		printf("\n\t Apellidos: %s - %s - %s - %s",x,z,y,v);
	}
	printf("\n\n");
...
//Concatenar los 2 apellidos de la segunda persona con el apellido menor de la primera persona
void pNMCASP() {
	...
	if (strcmp(firstLastName1,secondLastName1) < 0) {
		...
		strcpy(aux,firstLastName1);strcat(aux," ");strcat(aux,firstLastName2);
		strcat(aux," ");strcat(aux,secondLastName2);

		printf("\n\t Concatenado: %s",aux);
	} else {
		strcpy(aux,secondLastName1);strcat(aux," ");strcat(aux,firstLastName2);
		strcat(aux," ");strcat(aux,secondLastName2);
...
int numberOf4VocalWords(char x[100]){
	...
	if (token != NULL){
		...
		while(token != NULL){
			...
			suma = a+e+I+o+u;
			if ((a<=1)&&(e<=1)&&(I<=1)&&(o<=1)&&(u<=1)&&(suma==4)){
				printf(" %s ",token);c++;
			} 
			token = strtok(NULL, limitador);
			a = 0; e = 0; I = 0; o = 0; u = 0; suma = 0;
		}
	}
...
void showWordStartsWithVowel(char x[100]){
	...
	if (token != NULL){
		...
		while(token != NULL){
			...
			switch (token[0]){
				case 'a': printf("%s ",token); break;
				case 'e': printf("%s ",token); break;
				case 'i': printf("%s ",token); break;
				case 'o': printf("%s ",token); break;
				case 'u': printf("%s ",token); break;
				case 'A': printf("%s ",token); break;
				case 'E': printf("%s ",token); break;
				case 'I': printf("%s ",token); break;
				case 'O': printf("%s ",token); break;
				case 'U': printf("%s ",token); 
			}
